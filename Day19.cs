...
namespace Advent2021
{
    ...
    internal class Day19
    {
        ...
        const string _testData = @"--- scanner 0 ---
404,-588,-901
528,-643,409
-838,591,734
390,-675,-793
...
30,-46,-14";
        ...
        public void Test()
        {
            Assert.AreEqual((79, 3621), Run(FileHelpers.ReadAllLinesFromString(_testData)));
        }
        ...
        static (long,long) Run(IEnumerable<string> inputs)
        {
            ...
            foreach (var line in inputs)
            {
                ...
                else
                {
                    ...
                    currentScanner!.AddPoint(tokens[0], tokens[1], tokens[2]);
                }
            }

            scanners[0].Oriented = true;

            while (scanners.Any(s => !s.Oriented))
            {
                int foundCount = 0;
                var s0 = scanners[0];
            ...
            long manhattan = long.MinValue;
            foreach (var s0 in scanners)
            {
                foreach (var s1 in scanners)
                {
                    var d = Math.Abs(s0.Location.X - s1.Location.X) +
                            Math.Abs(s0.Location.Y - s1.Location.Y) +
                            Math.Abs(s0.Location.Z - s1.Location.Z);
                    if (d > manhattan) manhattan = d;
                }
            }
        ...
        class Scanner
        {
            ...
            public (int X, int Y, int Z) Location = (0, 0, 0);
